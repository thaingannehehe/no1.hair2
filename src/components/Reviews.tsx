...
function ReviewCard({ review }: { review: Review }) {
  return (
    <figure className="mb-6 flex w-full flex-col gap-5 rounded-xl border border-[#1C1C1C]/10 bg-white/60 p-8">
      <div className="flex gap-1">
        {Array.from({ length: 5 }).map((_, i) => (
          <span key={i} className="text-[14px] text-[#2A2A2A]">
            ★
          </span>
        ))}
      </div>
      <blockquote
        className="text-[15px] leading-[1.8] text-[#1C1C1C]"
      ...
      <figcaption className="mt-auto">
        <p
          className="text-[16px] text-[#1C1C1C]"
          style={{ fontFamily: "'Newsreader', serif", fontWeight: 400 }}
        >
          {review.name}
        </p>
        <p
          className="mt-1 text-[11px] uppercase tracking-[0.15em] text-[#767676]"
...
export default function Reviews() {
  return (
    <section
      ...
      className="overflow-hidden bg-[#F6F6F5] py-16 md:py-24"
    >
      <div className="container mx-auto px-6 sm:px-10 lg:px-16">
        ...
        <div className="relative">
          ...
          <div className="pointer-events-none absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-[#F6F6F5] to-transparent" />
          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-[#F6F6F5] to-transparent" />
        </div>
      </div>
    </section>
  );
